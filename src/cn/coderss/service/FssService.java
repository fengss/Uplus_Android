...
import cn.coderss.activity.BaseActivity;
import cn.coderss.activity.BaseActivity.BackPressHandler;
import cn.coderss.activity.LoginActivity;
import cn.coderss.activity.MainActivity;
import cn.coderss.R;
import cn.coderss.receiver.ImBroadcastReceiver;
import cn.coderss.receiver.ImBroadcastReceiver.EventHandler;
import cn.coderss.receiver.ImBroadcastReceiver;
...
public class FssService extends BaseService implements EventHandler,
		BackPressHandler {
	...
	private IBinder mBinder = new FssBinder();
	private IConnectionStatusCallback mConnectionStatusCallback;
	private SmackFss mSmackable;
	private Thread mConnectingThread,RegisterThread;
	private Handler mMainHandler = new Handler();
	private boolean mIsFirstLoginAction;
	// 自动重连 start
	private static final int RECONNECT_AFTER = 5;
	private static final int RECONNECT_MAXIMUM = 10 * 60;// 最大重连时间间隔
	private static final String RECONNECT_ALARM = "cn.coderss.RECONNECT_ALARM";
	...
	public IBinder onBind(Intent intent) {
		L.i(FssService.class, "[FSS_SERVICE] onBind");
	...
	public class FssBinder extends Binder {
		public FssService getService() {
			return FssService.this;
		}
	}
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		if (intent != null
				...
				&& TextUtils.equals(intent.getAction(),
						ImBroadcastReceiver.BOOT_COMPLETED_ACTION)) {
			String account = PreferenceUtils.getPrefString(FssService.this,
					PreferenceConstants.ACCOUNT, "");
			String password = PreferenceUtils.getPrefString(FssService.this,
	...
	public void Register(final String account,final String password){
		...
		RegisterThread = new Thread() {
			...
			public void run() {
				try {
					postConnecting();
					mSmackable = new SmackFss(FssService.this);
					if (mSmackable.Register(account, password)) {
						// 注册成功
						postRegisterScuessed();
					} else {
						// 注册失败
						postRegisterFailed();
					}
		...
		mConnectingThread = new Thread() {
			...
			public void run() {
				try {
					...
					L.i(FssService.class, "XMPPException in doConnect():");
					e.printStackTrace();
				} finally {
					if (mConnectingThread != null)
	...
	public void sendMessage(String user, String message) {
		L.i("消息发送－－－－>user:"+user+",message:"+message);
		if (mSmackable != null)
			mSmackable.sendMessage(user, message);
		else
			SmackFss.sendOfflineMessage(getContentResolver(), user, message);
	}
	...
	public void sendFile(String toUser,String filepath){
		L.i("文件发送－－－－>user:"+toUser+",filepath:"+filepath);
		if (mSmackable != null)
			mSmackable.sendFile(toUser, filepath);
	...
	private void connectionFailed(String reason) {
		L.i(FssService.class, "connectionFailed: " + reason);
	...
	//注册事件回调
	private void postRegisterScuessed() {
		mMainHandler.post(new Runnable() {
			public void run() {
				registerScuessed();
			}
	...
	//注册成功回调
	private void registerScuessed(){
		if (mConnectionStatusCallback != null)
			mConnectionStatusCallback.connectionStatusChanged(1,
					"success");
	}
	
	//注册失败事件回调
	private void postRegisterFailed() {
		mMainHandler.post(new Runnable() {
			public void run() {
				registerFailed();
			}
	...
	public void newMessage(final String from, final String message) {
		mMainHandler.post(new Runnable() {
			public void run() {
				if (!PreferenceUtils.getPrefBoolean(FssService.this,
						PreferenceConstants.SCLIENTNOTIFY, false))
					MediaPlayer.create(FssService.this, R.raw.office).start();
				if (!isAppOnForeground())
					notifyClient(from, mSmackable.getNameForJID(from), message,
							!mIsBoundTo.contains(from));
				// T.showLong(Service.this, from + ": " + message);
	...
	/**
	 * 基本用户各类activity onResume和onPause
	 * 状态准则
	 */
	// 判断程序是否在后台运行的任务
